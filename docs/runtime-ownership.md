...
| App launchers | Sway keybindings | Handy, Obsidian, LocalSend, Thunderbird, AppImage paths, `gtk-launch`, `notify-send` | desired/manual/AppImage/DNF mix | no | degraded application access | transitional | Obsidian and LocalSend are inventory-only desired AppImages; no modular installer exists. LocalSend, Handy, pCloud, and Proton Bridge are forced to floating windows on workspace 9 by Sway `app_id` rules. Waybar taskbar rewrites provide Nerd Font icons for these utility apps. |
...
| Brightness controls | Sway media keys, Waybar | `brightnessctl`, `notify-send` | `dnf` desired | no | degraded laptop brightness control | transitional | Sway media key bindings call wrapper scripts that notify and fail closed when `brightnessctl` is missing. Brightness decreases are clamped to a 7% minimum to avoid a black screen. The Waybar widget reads brightness through `brightness_status.sh` and supports scroll up/down for adjustment. |
...
| `brightnessctl` | brightness keybindings and Waybar status | mixed | Waybar brightness status warns when unavailable; adjustment helper scripts notify and exit when missing. |
