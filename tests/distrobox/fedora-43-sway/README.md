# Fedora 43 Distrobox Test

This target validates Fedora 43 package and CLI behavior before VM or machine testing. Fedora 43 is the supported Swaydora target for the current refactor state.

It is a container approximation of Fedora 43 Spin Sway. It does not validate a live Sway session, Waybar, portals, PipeWire sockets, display devices, or login manager behavior.
...
```bash
tests/distrobox/fedora-43-sway/host.sh
```

Create the container explicitly:

```bash
tests/distrobox/fedora-43-sway/host.sh --create
```

Run the validation:

```bash
distrobox enter swaydora-fedora-43-sway -- ./tests/distrobox/fedora-43-sway/run.sh
```

Run the real install inside the container:

```bash
distrobox enter swaydora-fedora-43-sway -- ./tests/distrobox/fedora-43-sway/install.sh --yes
```

Or from the host helper:

```bash
tests/distrobox/fedora-43-sway/host.sh --install
```

## What It Checks

- Fedora version is `43`.
...
`install.sh --yes` is different: it may install packages and enable COPRs inside the container. It uses `/tmp/swaydora-fedora43-install-home` as `HOME` by default so dotfile linking does not touch the host user's real config.
...
Fedora 44 remains unsupported until package and desktop component support is coherent enough for this setup.
